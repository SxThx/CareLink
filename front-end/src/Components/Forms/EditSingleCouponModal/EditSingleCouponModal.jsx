...
import "./EditSingleCouponModal.scss";
import { imageToBase64 } from "../../../utils/Base64Encoder";

const EditSingleCouponModal = ({
  isOpen,
  closeModal,
  campaign,
  handleEditCoupon,
  ...
  const [formData, setFormData] = useState({
    ...
    amount_of_coupons: campaign?.amount_of_coupons || 0,
    ...
    type: campaign?.type || "company_wide", // Default to "company_wide" if undefined
  ...
  const handleChange = (e) => {
    ...
    if (type === "file") {
      ...
      if (name === "promotion_category") {
        ...
        setFormData((prevData) => ({ ...prevData, promotion_category: selectedCategories }));
      } else if (name === "branch") {
        ...
        setFormData((prevData) => ({ ...prevData, branch: selectedBranches }));
      }
    } else if (name === "type") {
      setFormData((prevData) => ({
        ...prevData,
        type: value,
        branch: value === "company_wide" ? [] : prevData.branch, // Setting branch correctly
      }));
    } else {
  ...
  const handleSubmit = () => {
    const adjustedFormData = {
      ...formData,
      branch: formData.type === "company_wide" ? null : formData.branch, // Adjust branch before submission
    };
    console.log("Submitting formData:", adjustedFormData);
    handleEditCoupon(adjustedFormData, campaign.campaign_id, currentPage);
  };
...
export default EditSingleCouponModal;
